...
def spline_smoothing(data_dict, smoothing_factor=1):
    """
    使用样条插值对字典数据进行平滑。
    
    :param data_dict: 输入字典，键为日期字符串，值为数值
    :param smoothing_factor: 控制样条平滑程度的参数
    :return: 平滑后的字典
    """
    ...
    # 将平滑结果映射回字典
    smoothed_dict = {dates[i]: smoothed_values[i] for i in range(len(dates))}
...
if __name__ == "__main__":
    ...
    for company_short, industry_list in tqdm(company2industry_dict.items()):
        ...
        for industry in industry_list:
            company_meso_senti_list.append(np.array(list(industry_dict[industry].values())))
    ...
    bert_checkpoint = torch.load('./0.0 train_BERT/BERT_best_MSE.pth', map_location=torch.device('cpu'))
    BERT.load_state_dict(bert_checkpoint)

    mlp_checkpoint = torch.load('./0.0 train_BERT/MLP_best_MSE.pth', map_location=torch.device('cpu'))
    ...
    for company_short in tqdm(company_short_set):
        ...
        industry_list_of_company = company2industry_dict[company_short]
        for industry in industry_list_of_company:
            company_meso_senti = industry_dict[industry]
            company_micro_senti = add_dict_values(company_micro_senti, company_meso_senti)
        company_final_senti = spline_smoothing(company_micro_senti)
        company_final_senti_dict[company_short] = company_final_senti

        # bonds_result = all_bond_company_df[all_bond_company_df["发行人中文简称"] == company_short]
        # bonds_result_list = bonds_result["证券代码"].tolist()
        # for bond_code in bonds_result_list:
        #     bond_dict[bond_code] = company_final_senti
